...
module.exports = (recentlyCompletedTasks) => {
  const homeTab = HomeTab({ callbackId: 'tasks-home', privateMetaData: 'completed' }).blocks(
    Actions({ blockId: 'task-creation-actions' }).elements(
      Elements.Button({ text: 'Open tasks' }).value('app-home-nav-open').actionId('app-home-nav-open'),
      Elements.Button({ text: 'Completed tasks' }).value('app-home-nav-completed').actionId('app-home-nav-completed').primary(true),
